...
//=============================================================================
 /*:
 * @plugindesc v1.00 Battlers can be surrounded by an absorption barrier
 ...
 * @desc The default Barrier Penetratio Rate for all actions.
 * @default 0
 *
 * @param Default Penetration Flat
 * @desc The default Barrier Penetratio Flat for all actions.
 * @default 0
 *
 * @help
 * ============================================================================
 ...
 *   or -x. The Barrier Points altered for this notetag are unexpiring Barrier
 *   Points that do not remove themselves as time passes.
 *
 ...
 *   turns by +y or -y amount. These Barrier Points will expire after x turns.
 *   Each turn goes by during the battler's regeneration timing.
 *
 *   <Bypass Barrier>
...
Game_Battler.prototype.gainBarrier = function(value, turn) {
    if (!$gameParty.inBattle() && Yanfly.Param.ABRClear) return;
    this.initAbsorptionBarrier();
...
Game_Action.prototype.gainBarrierBonus = function(target, isTarget) {
    ...
    if (length <= 0) return;
    for (var i = 0; i < length; ++i) {
      target.gainBarrier(barriers[i] || 0, i);
      target.gainBarrierEval(evalBarriers[i] || '', i, this.subject(), target);
    }
    target.refresh();
};
